...
**Edge cases:**
- Network failure: "Couldn't send report. Results saved locally. Try again with `mcp-audit report --email <email>`"
...
# Generate PDF locally without sending email
mcp-audit scan --pdf -o report.pdf

# Send report to specific email (non-interactive)
mcp-audit scan --email raj@acme.com

# Resend report for a previous scan (if user skipped initially)
mcp-audit report --email raj@acme.com
```
...
| `--pdf -o <file>` | Generate PDF locally, save to file. No email prompt. |
| `--email <email>` | Non-interactive. Send report to this email. No prompt. |
...
│  └─────────────────────────────────────┘                           │
│                                                                     │
│  Or: [Download PDF] [Download JSON] [Download CSV] [Download MD]   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

**Behavior:**
- Section appears below scan results (after existing export buttons)
- "Send Report" button:
  - Validates email
  - POSTs to backend
...
- "Download PDF" button:
  - Generates PDF client-side (no backend needed)
  - Downloads directly to user's machine
  - No email collected (user chose privacy)

**Client-side PDF generation:**
- Use html2pdf.js or jsPDF library
- Generate from existing results DOM
- No backend dependency for download-only flow
...
|--------|----------|------------|
| **Client-side PDF** | "Download PDF" button (no email) | html2pdf.js with custom CSS |
| **Server-side PDF** | "Send Report" email delivery | Puppeteer or Playwright (renders HTML template) |

**Recommendation:** Use server-side PDF generation for emailed reports (consistent, professional rendering). Use client-side for instant download option.
...
| File | Changes |
|------|---------|
| `cli.py` | Add email prompt after scan, add `--no-report`, `--pdf`, `--email` flags |
| `report.py` (new) | PDF generation logic using reportlab or weasyprint |
| `upload.py` (new) | API client to POST to backend |
| `templates/report.html` (new) | HTML template for PDF generation |

### Web UI

| File | Changes |
|------|---------|
| `index.html` | Add email input + "Send Report" button after results |
| `app.js` | Add email capture logic, client-side PDF generation, backend POST |
| Add html2pdf.js or jsPDF | Client-side PDF library |

### Backend (New)

| File | Purpose |
|------|---------|
| `api/mcp-leads.js` | Serverless function: receive POST, store lead, trigger email |
| `lib/pdf.js` | PDF generation from scan summary |
...
3. **Backend: Lead storage endpoint** (1-2 hrs) — POST to Supabase/Airtable
4. **Backend: Email sending** (1 hr) — Send confirmation email (no PDF yet)
5. **PDF template** (2-3 hrs) — HTML template that looks professional
6. **Backend: PDF generation** (1-2 hrs) — Generate PDF server-side
7. **CLI: Wire to real backend** (30 min) — Connect prompt to live endpoint
8. **Web UI: Email capture** (1 hr) — Add input + button + backend call
9. **Web UI: Client-side PDF download** (1-2 hrs) — Download without email
10. **Testing + polish** (1-2 hrs)
...
- [ ] `--pdf -o <file>` generates local PDF without network call
...
- [ ] "Download PDF" button generates PDF client-side (no email required)
- [ ] Loading state while sending/generating

### Backend
- [ ] `POST /api/mcp-leads` receives and stores lead
- [ ] Email format validation
- [ ] PDF generated from scan summary
